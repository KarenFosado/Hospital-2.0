...
def get_user_by_creentials(db: Session, username: str, correo: str, telefono: str, password: str):
    return db.query(models.users.User).filter((models.user.User.Nombre_Usuario == username)|
                                              (models.user.User.Correo_Electronico == correo) |
                                              (models.user.User.Numero_Telefonico_Movil == telefono),
                                              models.user.User.Contrasena == password).first()
