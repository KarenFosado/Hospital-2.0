...
def read_usersrols(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    db_usersrols= crud.usersrols.get_user(db=db, skip=skip, limit=limit)
    return db_usersrols
...
def read_rol(id_user: int, id_rol: int, db: Session = Depends(get_db)):
    db_userrol= crud.usersrols.get_user(db=db, id_user=id_user,id_rol=id_rol)
...
def create_user(userrol: schemas.usersrols.UserRolCreate, db: Session = Depends(get_db)):
    db_userrol = crud.usersrols.get_user(db=db, id_user=userrol.Usuario_ID, id_rol=userrol.Rol_ID)
    print (db_userrol)
    if db_userrol:
        raise HTTPException(status_code=400, detail="Usuario existente intenta nuevamente")
    return crud.usersrols.create_user(db=db, userrol=userrol)
...
def update_user(id_user: int, id_rol: int, userrol: schemas.usersrols.UserRolUpdate, db: Session = Depends(get_db)):
    db_userrol = crud.usersrols.update_user(db=db, id_user=id_user, id_rol=id_rol, userrol=userrol)
...
def delete_rol(id_user: int, id_rol: int, db: Session = Depends(get_db)):
    db_userrol = crud.usersrols.delete_user(db=db, id_user=id_user, id_rol=id_rol)
    if db_userrol is None:
